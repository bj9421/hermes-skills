...
**Prefer direct CLI invocation** (the bundled Python/Node.js/shell scripts in `scripts/`).

**Fallback — REST API via cURL:** When the CLI scripts are not on `$PATH`, are missing dependencies, or the environment doesn't have Python/Node.js, you can call the API directly via HTTP. See `references/rest-api-direct.md` for the exact `curl` command shapes, response format, and Python parsing recipe. The key endpoint is `GET/POST https://api.anysearch.com/v1/search` — **not** `/v1/chat/completions` (which returns 404).

**Beyond search — data collection:** AnySearch `extract` can serve as a **free alternative to paid scraping APIs** (Apify, SerpAPI) for structured data collection, **but only when the target page serves data as static HTML** (not JS-rendered SPA). See `references/data-collection-patterns.md` for the architecture pattern, pre-flight checklist to test feasibility, and real-world case studies including Instagram (which blocks extract entirely). Key insight: if the data is server-side rendered in the HTML response, `extract` + SQLite INSERT OR REPLACE is often zero-cost vs $5+/month API subscriptions. **Always run the pre-flight test first** (extract the page and check if target data is in the output) before committing to a parsing approach.
...
> ⚠️ **Dashboard key ≠ cron key:** API keys set in the Hermes dashboard only affect the gateway process. Cron jobs, memory scanners, and other background tasks run in isolated sessions that DO NOT inherit the gateway's env vars. **Always set API keys in `.env`** for them to be available everywhere.
