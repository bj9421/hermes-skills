...
46. **🔴 PPT 階段 2+3 — 版型系統（2026-08-06 ✅）** — **階段 2**（輸出結構）：`_extract_key_points` prompt 加 `slide_type` 欄位 + 版型規則（hook/content/data/quote/qa/action）。**階段 3**（渲染升級）：新增 5 個版型渲染函數 — `_add_hook_slide`（開場大標題置中，無 badge）、`_add_quote_slide`（金句置中 + 「「」裝飾）、`_add_data_slide`（數字卡動態寬度：n=2 時 4.1"，n≥3 時均分 8.8"）、`_add_qa_slide`（3 問題❓ + CTA →）、`_add_action_slide`（行動步驟→）。dispatch 依 `slide_type` 選版型。修既有 bug：content 版型 bullets 容器太高（4→2.6 inch）→ 超界。**驗證**：幾何超界 0 + LLM 端到端（Cherry Studio V2 → 7 頁，版型分佈 hook:1+content:3+data:1+quote:1+action:1 ✅ 無缺版型）+ 118 tests + Playwright 手機模式（_v3.pptx 路徑顯示 + 無破版）✅。