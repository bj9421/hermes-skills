...
## Delaying / rescheduling tasks

When a user wants to **delay a running task** to a specific future date (e.g. "move this to next Tuesday because the data won't be ready until then"):

1. **Kill the worker** — `kill <pid>` or wait for it to time out.
2. **Pause the task** — set `status='paused'` in kanban.db so the dispatcher won't re-claim it:
   ```python
   cur.execute("UPDATE tasks SET status='paused', claim_lock=NULL, worker_pid=NULL WHERE id=?", [task_id])
   ```
3. **Schedule one-shot cron to unpause** — create a `no_agent=True` cron job that fires at the target time. The script updates `status='ready'` and resets claim fields:
   - Write a small Python script in `~/.hermes/scripts/` that does the DB update.
   - Create the cron job with `cronjob(action='create', script='scriptname.py', no_agent=True, schedule='ISO_TIMESTAMP')`.
   - The cron fires once; the script flips `status='ready'`; the dispatcher picks it up within 60s.

   Example script (`kanban_unpause.py`):
   ```python
   import sqlite3
   conn = sqlite3.connect('/opt/data/kanban.db')
   conn.execute("UPDATE tasks SET status='ready', claim_lock=NULL, worker_pid=NULL, consecutive_failures=0 WHERE id=? AND status='paused'", [TASK_ID])
   conn.commit()
   conn.close()
   ```

   > ⚠️ **paused, not ready.** Setting status='ready' immediately causes the dispatcher to re-claim the task before you can schedule the delayed pickup. Always pause first, then let the cron job un-pause at the right moment.

4. **Verify** — confirm `status='paused'` in DB, and the cron job shows `state='scheduled'` with the correct `next_run_at`.
