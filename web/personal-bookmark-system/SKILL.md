...
**v15.7（2026-08-08，commit `b3d6bf7`）：標籤與 radio 分列兩行** — 每個設定項（PPT 配色、口播長度）的標籤單獨一行，radio 另起一行。HTML：4 個 nh-scheme-row（標籤 ×2 + radio ×2）。CSS：flex column 堆疊。
...
- **2026-08-07 深夜終局（v10→v13）**：v10（全部 job 顯示佇列頁籤）→ v11（三態模型：SETUP_MODE flag 判配置階段）→ **v12（commit `4d8f130`：頁籤「工作佇列/工作進度」重定義 + 送出後自動切 progress + PPT/圖卡 checkbox）** → **v12.1（started_at COALESCE bug 修正）** → **v13（處理中顯示「已處理 X 分鐘」+ 滑動動畫）**；117 tests 全綠
- **2026-08-07 腳本生成 bug 修復**：`podcast.py` 新增 TTS 短 chunk 過濾（pitfall 52）+ prompt 標點符號分段要求（pitfall 53）。Job #84 修復後 arts=2, paths=2 ✅
