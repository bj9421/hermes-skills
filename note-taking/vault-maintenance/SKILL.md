...
7. **簡體/繁體 duplicate folders in Holographic/** — when a cron export changed naming convention (e.g. `每日检讨` → `每日檢討`), the old 簡體 folder lingers as a stale copy. Pattern (2026-08-05): `Holographic/每日检讨/` had 1 old file (684 B, 18 jobs summary) while `Holographic/每日檢討/` had 5 complete files (11 KB, newer+fuller). Resolution: 簡體 folder is the older/less-complete generation → delete the whole 簡體 folder after showing user the comparison table (大小/內容/時間); Holographic is a cron-exported snapshot so deletion loses nothing (DB re-exports next run). **Always get explicit user approval before `rm -rf`** — user's cleanup rule: never delete user-visible folders without consent.
